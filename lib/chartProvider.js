	...
	if(p){
		...
		for (client of clients){
			check_DB.getChartsFromClient(db, client.id).then(function(listChart){
				...
				mailer.setMailOptions(client.Email, "CheckAnywhere", db.name, listChart);
				mailer.sendMail();
...
async function sendMailsCip(db, charts, clients){
	...
	if(promise){
		...
		for (client of clients){
			...
			cip_DB.getChartsFromClient(db, client.idClient).then(function(listToConvert){

				chartCipListConvert(listToConvert, charts).then(function(listChart){

					// Envoi du mail en incluant les graphiques
					mailer.setMailOptions(client.email, "CIPAnywhere", db.name, listChart);
					mailer.sendMail();
