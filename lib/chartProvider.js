...
async function loopSerieCheck(db, chart, chartData, series){
	...
	for (serie of series){
	    ...
	    if(serie.Objectif_Val != null){
	    	chart.addPlotline(serie.Objectif_Libelle, serie.Couleur, serie.Objectif_Val);
	    }

	    var promise_Data = await check_DB.getSeriesData(db, serie["Monetaire"], serie["Ratio"], chartData["Periode"], idData, serie["id"], cumul).then(function(result){
